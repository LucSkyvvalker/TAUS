...
def extractor(df, unigramSrc,bigramSrc, trigramSrc, unigramTgt,bigramTgt,trigramTgt,
    ...
    for i in range(len(df)):
        ...
        if i % 100 == 0:
            ...
            string = str(i) + '/' + str(t0) + ', elapsed time: ' + str(timeCurrent - timeStart)
            print(string, end="\r")
        source = str(df['source'][i])
    ...
    dfnew['wordCountSrc'] = df['source_wc']
    dfnew['wordCountTgt'] = df['target_wc']
    dfnew['wordCountDif'] = abs(df['source_wc'] - df['target_wc'])
    return dfnew
